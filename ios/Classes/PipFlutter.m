    ...
    if (context == timeRangeContext) {
        if (_eventSink != nil) {
            ...
            for (NSValue* rangeValue in [object loadedTimeRanges]) {
                ...
                int64_t start = [PipFlutterTimeUtils FLTCMTimeToMillis:(range.start)];
                int64_t end = start + [PipFlutterTimeUtils FLTCMTimeToMillis:(range.duration)];
                if (!CMTIME_IS_INVALID(_player.currentItem.forwardPlaybackEndTime)) {
                    int64_t endTime = [PipFlutterTimeUtils FLTCMTimeToMillis:(_player.currentItem.forwardPlaybackEndTime)];
                    if (end > endTime){
                        end = endTime;
                    }
                }
...
- (void)onReadyToPlay {
    if (_eventSink && !_isInitialized && _key) {
        ...
        int64_t duration = [PipFlutterTimeUtils FLTCMTimeToMillis:(_player.currentItem.asset.duration)];
